...
@patch(target="src.api.v1.router.auth")
@patch(target="src.api.v1.router.APIRouter")
def test_get_api_router(mock_api_router, mock_auth, mock_user, mock_health_check):
    ...
    assert mock_router. \
        include_router.mock_calls == [
            call(router=mock_auth.router, prefix="/auth",),
