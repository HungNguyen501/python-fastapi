...
class AuthService:
    ...
    def __init__(self, user_service: UserService = Depends(), redis_pool: RedisPool = Depends(get_redis_pool)):
        """Constructor"""
    ...
    async def create_user(self, data: UserCreate,):
        ...
        new_user = await self.user_service.create(data=data)
    ...
    async def authenticate(self, username: str, password: str,):
        ...
        if user_cache := await self.redis_pool.get(key=username):
            ...
            user_data = await self.user_service.get(uuid=user_cache["uuid"])
